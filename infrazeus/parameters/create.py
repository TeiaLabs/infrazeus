...
def detect_secrets_with_ai(service_variables: dict[str, str]):
    secret_keywords = [
        "mongodb_uri", "password", "api_key", "secret", "token", "key"
    ]
    secret_dict = {key: value for key, value in service_variables.items()
                   if any(keyword in key.lower() for keyword in secret_keywords)}
    return secret_dict


#%%
def create_secret(service: Service, service_variables: Dict[str, str]) -> Optional[dict[str, Any]]:
...
def create_parameters(service: Service, service_variables: Dict[str, str]) -> Optional[Dict[str, Any]]:
